...
const nextId = (tool: string): string => {
  ...
  const prefix = prefixes[tool] ?? 'FND';
...
const RULES: Rule[] = [
  // ── Aderyn SAST ──────────────────────────────────────────────────────────
  {
    ...
    description:
      'An account is used without validating its owner. An attacker can substitute an arbitrary account.',
    remedy:
      'Add `constraint = vault.owner == user.key()` or use Anchor `#[account(has_one = owner)]`.',
  },
  {
    tool: 'aderyn',
    severity: 'critical',
    ...
    description:
      'Instruction handler accepts a context without requiring a signer, allowing unauthorized invocations.',
    remedy:
      'Add a `Signer<\'info>` account to the accounts struct and verify it in the handler.',
  },
  {
    tool: 'aderyn',
    severity: 'high',
    ...
    description:
      'Direct arithmetic on an account field without checked/saturating math can overflow or underflow.',
  ...
  },
  {
    ...
    description:
      'A program ID is hardcoded. Deployments to devnet/mainnet will require manual updates.',
    remedy:
      'Use an environment variable or config file to manage program IDs per environment.',
  },
  {
    tool: 'aderyn',
    severity: 'info',
    ...
    description:
      'An incomplete or flagged code comment was found. Ensure it is resolved before production.',
  ...
  // ── Clippy ────────────────────────────────────────────────────────────────
  {
    ...
    description:
      'An unchecked multiplication can silently overflow, producing incorrect results.',
  ...
  },
  {
    ...
    description:
      'Using integer boundaries without explicit boundary validation may cause panics.',
    remedy:
      'Add range validation before using boundary constants in arithmetic.',
  },
  {
    tool: 'clippy',
    severity: 'low',
    ...
    description:
      '`.unwrap()` panics on `None`/`Err`, crashing the program without a meaningful error.',
  ...
  // ── cargo-audit (dependency vulnerabilities) ──────────────────────────────
  {
    ...
    description:
      'Versions below 4.0 of curve25519-dalek have a timing side-channel vulnerability (RUSTSEC-2024-0012) that can leak private key material.',
    remedy: 'Upgrade to `curve25519-dalek = "4.1"` and audit downstream usages.',
    fileFilter: (f: string) => f === 'Cargo.toml',
  },
  {
    tool: 'cargo-audit',
    severity: 'medium',
    ...
    description:
      'Versions 1.10–1.15 of solana-program have known RPC and account-deserialization bugs.',
  ...
  },
  {
    ...
    description:
      'Older Anchor versions lack important safety checks and IDL improvements.',
  ...
  // ── cargo-geiger (unsafe blocks) ─────────────────────────────────────────
  {
    ...
    description:
      'An `unsafe` block bypasses Rust\'s memory-safety guarantees. Any bug inside can lead to undefined behavior.',
    remedy:
      'Eliminate the unsafe block where possible. If unavoidable, add a `// SAFETY:` comment justifying correctness.',
  },
  {
    tool: 'cargo-geiger',
    severity: 'medium',
    ...
    description:
      'An unsafe function requires every caller to uphold invariants that the compiler cannot verify.',
 ...
 * @param filename - File name (used for scoping rules)
 * @returns Finding[]
 */
