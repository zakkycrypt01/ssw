...
const SEV_COLORS = {
  critical: { bg: '#2a0a0a', border: '#7f1d1d', text: '#fca5a5', label: '#ef4444' },
  high:     { bg: '#2a1300', border: '#7c2d12', text: '#fdba74', label: '#f97316' },
  medium:   { bg: '#2a1f00', border: '#713f12', text: '#fde68a', label: '#eab308' },
  low:      { bg: '#0a1f0a', border: '#14532d', text: '#86efac', label: '#22c55e' },
  info:     { bg: '#0a0f2a', border: '#1e3a5f', text: '#93c5fd', label: '#3b82f6' },
};

export const generatePDF = (findings, projectName = 'Unknown Project', toolResults = {}) => {
  const summary = summarizeFindings(findings);
  const now = new Date();
  const scannedAt = now.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
  const tools = Object.keys(toolResults).join(', ') || 'cargo-audit, cargo-geiger, clippy, aderyn';
...
<body>
  ...
  <div style="display:flex;gap:16px;margin-bottom:32px;">
    ${['critical','high','medium','low','info'].map(sevRow).join('')}
  </div>
